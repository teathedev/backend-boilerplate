...
type Client struct {
	...
	// AccessTokenKey is the client for interacting with the AccessTokenKey builders.
	AccessTokenKey *AccessTokenKeyClient
...
func (c *Client) init() {
	...
	c.AccessTokenKey = NewAccessTokenKeyClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		ctx:            ctx,
		config:         cfg,
		AccessTokenKey: NewAccessTokenKeyClient(cfg),
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.AccessTokenKey.Intercept(interceptors...)
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *AccessTokenKeyMutation:
		return c.AccessTokenKey.mutate(ctx, m)
...
// AccessTokenKeyClient is a client for the AccessTokenKey schema.
type AccessTokenKeyClient struct {
	config
}

// NewAccessTokenKeyClient returns a client for the AccessTokenKey from the given config.
func NewAccessTokenKeyClient(c config) *AccessTokenKeyClient {
	return &AccessTokenKeyClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `accesstokenkey.Hooks(f(g(h())))`.
func (c *AccessTokenKeyClient) Use(hooks ...Hook) {
	c.hooks.AccessTokenKey = append(c.hooks.AccessTokenKey, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `accesstokenkey.Intercept(f(g(h())))`.
func (c *AccessTokenKeyClient) Intercept(interceptors ...Interceptor) {
	c.inters.AccessTokenKey = append(c.inters.AccessTokenKey, interceptors...)
}

// Create returns a builder for creating a AccessTokenKey entity.
func (c *AccessTokenKeyClient) Create() *AccessTokenKeyCreate {
	mutation := newAccessTokenKeyMutation(c.config, OpCreate)
	return &AccessTokenKeyCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of AccessTokenKey entities.
func (c *AccessTokenKeyClient) CreateBulk(builders ...*AccessTokenKeyCreate) *AccessTokenKeyCreateBulk {
	return &AccessTokenKeyCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *AccessTokenKeyClient) MapCreateBulk(slice any, setFunc func(*AccessTokenKeyCreate, int)) *AccessTokenKeyCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &AccessTokenKeyCreateBulk{err: fmt.Errorf("calling to AccessTokenKeyClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*AccessTokenKeyCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
// Update returns an update builder for AccessTokenKey.
func (c *AccessTokenKeyClient) Update() *AccessTokenKeyUpdate {
	mutation := newAccessTokenKeyMutation(c.config, OpUpdate)
	return &AccessTokenKeyUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *AccessTokenKeyClient) UpdateOne(_m *AccessTokenKey) *AccessTokenKeyUpdateOne {
	mutation := newAccessTokenKeyMutation(c.config, OpUpdateOne, withAccessTokenKey(_m))
	return &AccessTokenKeyUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *AccessTokenKeyClient) UpdateOneID(id uuid.UUID) *AccessTokenKeyUpdateOne {
	mutation := newAccessTokenKeyMutation(c.config, OpUpdateOne, withAccessTokenKeyID(id))
	return &AccessTokenKeyUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for AccessTokenKey.
func (c *AccessTokenKeyClient) Delete() *AccessTokenKeyDelete {
	mutation := newAccessTokenKeyMutation(c.config, OpDelete)
	return &AccessTokenKeyDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *AccessTokenKeyClient) DeleteOne(_m *AccessTokenKey) *AccessTokenKeyDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *AccessTokenKeyClient) DeleteOneID(id uuid.UUID) *AccessTokenKeyDeleteOne {
	builder := c.Delete().Where(accesstokenkey.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &AccessTokenKeyDeleteOne{builder}
}

// Query returns a query builder for AccessTokenKey.
func (c *AccessTokenKeyClient) Query() *AccessTokenKeyQuery {
	return &AccessTokenKeyQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeAccessTokenKey},
		inters: c.Interceptors(),
	}
}

// Get returns a AccessTokenKey entity by its id.
func (c *AccessTokenKeyClient) Get(ctx context.Context, id uuid.UUID) (*AccessTokenKey, error) {
	return c.Query().Where(accesstokenkey.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *AccessTokenKeyClient) GetX(ctx context.Context, id uuid.UUID) *AccessTokenKey {
...
func (c *AccessTokenKeyClient) Hooks() []Hook {
	return c.hooks.AccessTokenKey
}

// Interceptors returns the client interceptors.
func (c *AccessTokenKeyClient) Interceptors() []Interceptor {
	return c.inters.AccessTokenKey
}

func (c *AccessTokenKeyClient) mutate(ctx context.Context, m *AccessTokenKeyMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&AccessTokenKeyCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&AccessTokenKeyUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&AccessTokenKeyUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&AccessTokenKeyDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown AccessTokenKey mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		AccessTokenKey, RefreshToken, User []ent.Hook
	}
	inters struct {
		AccessTokenKey, RefreshToken, User []ent.Interceptor
	}
)
